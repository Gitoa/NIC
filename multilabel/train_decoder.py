...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (images, captions, lengths) in enumerate(data_loader):
            ...
            outputs = decoder(result['embedded_features'], captions, lengths)
            ...
            if (i+1) % args.save_step == 0:
                torch.save(decoder.state_dict(), os.path.join(args.model_path, 'decoder-{}-{}.pt'.format(epoch+1, i+1)))
        end = time.time()
        #if epoch % 3 == 1:
        torch.save(decoder.state_dict(), os.path.join(args.model_path, 'decoder-epoch-{}.pt'.format(epoch+1)))
