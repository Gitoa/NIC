...
def multilabel_loss(class_logits, labels, device, freq):
    # type: (Tensor, Tensor)
    """
    ...
    """
    weight = {
        'V':torch.Tensor([62]).to(device),
        'N':torch.Tensor([105]).to(device),
        'J':torch.Tensor([50]).to(device)
    }
    """
    m = torch.nn.LogSigmoid()
    probs = {}
    for key in class_logits:
        probs[key] = F.sigmoid(class_logits[key])
        weight = np.sqrt((82783 - freq[key]) / (freq[key]))
        pos_weight = torch.from_numpy(weight)
        loss = F.binary_cross_entropy_with_logits(class_logits[key], labels[key].to(device), pos_weight=pos_weight.to(device))
        #loss = F.multilabel_soft_margin_loss(class_logits[key], labels[key].to(device))
        losses = losses + loss
    return losses, probs
...
class MultilabelRoIHeads(torch.nn.Module):
    """
    __annotations__ = {
    ...
    def __init__(self,
        ...
        super(MultilabelRoIHeads, self).__init__()
        ...
        self.box_head_v = box_head['V']
        self.box_head_n = box_head['N']
        self.box_head_j = box_head['J']

        self.box_predictor_n = box_predictor['N']
        self.box_predictor_j = box_predictor['J']
        self.box_predictor_v = box_predictor['V']
    ...
    def forward(self, features, classification_features, proposals, image_shapes, targets=None):
        ...
        box_features = {
            'V': self.box_head_v(box_features),
            'J': self.box_head_j(box_features),
            'N': self.box_head_n(box_features)
        }

        classification_features = classification_features.flatten(start_dim=1)

        classification_n = self.fc_n(classification_features).reshape(-1, 1, vec_n_len)
        classification_v = self.fc_v(classification_features).reshape(-1, 1, vec_v_len)
        classification_j = self.fc_j(classification_features).reshape(-1, 1, vec_j_len)

        class_logits = {
            'V': torch.cat((self.box_predictor_v(box_features['V'])[0].reshape(-1, proposal_num, vec_v_len), classification_v), 1),
            'J': torch.cat((self.box_predictor_j(box_features['J'])[0].reshape(-1, proposal_num, vec_j_len), classification_j), 1),
            'N': torch.cat((self.box_predictor_n(box_features['N'])[0].reshape(-1, proposal_num, vec_n_len), classification_n), 1)
        }
        # There are 50 proposals for each image, maxpooling to get the multilabel scores
        for key, v in class_logits.items():
            v = torch.max(v, 1)[0]
            class_logits[key] = v

        #To get the probability, assign the class_logits to Sigmoid
        ...
        if self.training:
            assert targets is not None
            loss_classifiers, probs = multilabel_loss(
                class_logits, targets, device, self.freq)
            losses = {
                "loss_classifiers": loss_classifiers,
            }
        else:
            #boxes, scores, labels = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes)
            """
            TO DO: change the output in test time
            """
            embedded_features = torch.cat((class_logits['V'], class_logits['J'], class_logits['N']), 1)
            result = {
